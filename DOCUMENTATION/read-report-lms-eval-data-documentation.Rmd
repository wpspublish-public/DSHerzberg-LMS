...
```
As in the previous code chunk, we call `mutate(across())` to apply formatting to the `_pct` columns. Here, however, we have to invoke `ungroup()` to remove the grouping applied earlier in the pipeline, because grouping can interfere with the function of `across()`. We did not need to call `ungroup()` in the previous code chunk, because grouping apparently was removed in that pipeline by `separate()`.
```{r freq, echo = 76:83, eval = FALSE}
```
Because we intend to join `freq_table_rhs_num_cols` and `freq_table_super_sub_cols` into a single frequency table, we must ensure that these two objects have identical column structure and formatting. The next call of `mutate()` modifies `freq_table_rhs_num_cols` to meet this requirement, adding an empty `sub_q` column, and using `across(c(value, freq, total), ~ as.character(.))` to coerce a certain subset of columns to character. We use `relocate()` to put the columns into the desired sequence.
