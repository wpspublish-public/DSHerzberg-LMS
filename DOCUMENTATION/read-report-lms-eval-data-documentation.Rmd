...
* For data cleanup, we prefer elegant, generalizeable solutions, but for expediency's sake we sometimes make do with hard-coded work-arounds. The snippet below contains an example of such a work-around; we use `stringr::str_replace_all()` to strip out all different versions of the unneeded value labels for the sub-ordinate question numerical responses. These versions are stated explicitly in the code, a classic, non-generalizable, hard-coded solution. The desired recoding (or stripping, in which we replace a certain pattern with `""`) can be passed to `str_replace_all()` in the form of a vector of pattern-replacement pairs (e.g, `" [|] Excellent" = ""`).
...
```{r prep, echo = 167:176, eval = FALSE}
```
