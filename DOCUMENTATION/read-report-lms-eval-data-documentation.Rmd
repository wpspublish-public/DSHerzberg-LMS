...
As previously, we use a logical test to identify the rows to recode. Here that test is expressed in the predicate `lag(super_q) == super_q`, which returns `TRUE` for all rows where the value of a cell in `super_q` is identical to the value of the immediately preceding cell. The only row where that predicate returns `FALSE` is the first row-wise instance of a particular super-ordinate question. For those first instances, the preceding cell is either a different super-ordinate question, or `NA` (in the first cell at the top of the column). `TRUE ~ super_q` is the "else" condition,  indicates that when the predicate returns `FALSE`, the existing value of `super_q` is retained. 

The end result is that the question text is retained in `super_q` only where it first appears, at the top of the set of rows that are associated with that particular super_ordinate question. `super_q` is cleared of values in the remainder of that set of rows, creating a more readable table.
```{r freq, echo = 46:50, eval = FALSE}
```
Within `mutate()`, `across()` designates a subset of columns to receive the same formatting or other functional processing. In the next snippet, we format the three `_pct` columns, listing them in a vector that is the first argument to `across()`. The second argument to `across()` is the formatting function, set off with the formula notation `~`.
...
* `.`: a stand-in for the columns identified by the first argument to `across()`;
