...
* `|`: the disjunctive operator joins the two calls of `str_detec()`, so that the compound predicate returns `TRUE` for any row contain _either_ of the two substrings, and, consequently, either of two subordinate questions that share the same value-label mapping. Thus, another coding efficiency is gained: one set of predicates can be used to apply a certain value-label mapping to all of the super-ordinate questions with which it is associated.
...
```
As in the previous code chunk, we call `mutate(across())` to apply formatting to the `_pct` columns. Here, however, we have to invoke `ungroup()` to remove the grouping applied earlier in the pipeline, because `across()` often throws an error when it encounters grouped data. We did not need to call `ungroup()` in the previous code chunk, because grouping (apparently) was removed in that pipeline by `separate()`.
