...
Overall educational experience: 5 | Excellent, Accessibility of platforms/materi…
```
The column name, `Access/Setting/Overall Experience` is a super-ordinate question. The row values going down the column contain the sub-ordinate questions (e.g., `Overall educational experience`, `Accessibility of platforms/materials`, etc.), the numerical responses to those sub-ordinate questions, and the text label corresponding to each numerical response (e.g., `5 | Excellent`. This represents a classic "untidy" data input, and this code cleans it up by creating separate columns for each super- and sub-ordinate question pair, with the rows containing _only_ the numerical responses by person (thus omitting the text labels).
...
```{r script, echo = 30:71, eval = FALSE}
```
