...
This is a method for reading output from WPS LMS system, cleaning up and reformatting the data, and producing a summary report of the course evaluation data. The final report is a .xlsx workbook with two sheets:

* `text responses`: Identifying information on the course participants, plus each participant's responses to the open-ended, text-response questions;
* `freq tables`: Frequency tables with response counts and other summary measures for questions with numerically coded, fixed-choice responses.
...
We use `map()`, with `list_super_sub_cols` as the `.x` element, to transform one data frame at a time. We pass each data frame to `select(contains("r"))`, thus isolating only the columns containing the numerical responses. Using `mutate(across(everything))` we apply a function to all columns in the `.x` input. The cells of these columns contain numbers formatted as strings. We strip out white space with `str_replace(., " ", "")`, and coerce the result to integer with `base::as.integer()`. The code returns `list_r_cols`, a list of data frames holding the properly formatted numerical response columns for each super-ordinate question.
```{r format, echo = 16:23, eval = FALSE}
```
In the data frames held by `list_super_sub_cols`, the sub-ordinate question text exists as duplicate cell values in the `q` columns. We need to transfer these strings into a vector so they can be used in the column names for the final output. As before, we `map()` over `list_super_sub_cols`, using the `.x` argument to pass one data frame per iteration to a series of functions that executes the required transformation.

We use `select(contains("q"))` to isolate the columns containing the sub-ordinate question text. Because every cell of these columns contains identical strings, we capture only the first row with `filter(row_number() == 1)`, coercing this single row to a character vector with `as.character()`.
...
In the snippet below, we initialize `freq_table_super_sub_cols` to hold the frequency table for the super-sub-ordinate questions. We extract the names of the relevant columns into a character vector with `names(named_super_sub_r_cols)`, and segregate the columns for analysis by wrapping their names in `select(all_of())`. 

`tidyr::pivot_longer()` transforms the data object from wide to long (nested) format. The arguments to `pivot_longer()` include the tidyselect helper `everything()`, which indicates that all columns will be included in the transformation, and the names of the new destination columns for the input column names (`names_to`) and the input cell values (`values_to`). `pivot_longer()` returns a nested data frame with two columns, `item` and `value`. 

In this long object, the items and their respective response values are nested within each person. In the original input, there was a single row per person, and the item responses ran _horizontally_, across columns, in the cells of that row. In the transformed long object, each person has multiple rows, one for each item. The item responses now run _vertically_, within the `value` column, with items repeating for the rows of each successive person.
...
```
The range of possible response values for the super-sub-ordinate questions is `1` to `5`. In the questions shown in the this example, no person chose responses `1` or `2`, so there are no rows corresponding to these two numerical responses. We must nevertheless report the person counts for `1` and `2` in the frequency table output. We use `tidyr::complete(value = 1:5)` to create rows for any numerical responses not chosen by any person. `group_by(item)` ensures that missing rows are created separately for each unique value of `item`.
...
```
The next snippet applies further modifications to the output. `arrange()` sorts the table in `desc()` order by `value`, retaining the existing group structure (`.by_group = TRUE`) so that sorting on response value is executed _within_ each item's five response-value rows. In the summary table, the substantive meaning of `NA` is "0 person responses". We use `tidyr::replace_na()` to recode `NA` to `0`, passing a list of columns and replacement values (`list(n = 0)`). `mutate()` creates new columns (e.g., `total_pct`, `valid_pct`, etc.) that are needed for the final output. We wrap variable definitions in `round()` to apply required numerical formatting.
```{r freq, echo = 7:14, eval = FALSE}
```
In the current data object, super- and subordinate question pairs exist as a single long string in the `item` column. The frequency table output requires that each element of the pair reside in its own column. As described previously, we call `tidyr::separate()` to divide up these long strings and send them to discrete destination columns. Here, the third argument specifies that the strings in `item` will be split on the string `:_` (which will be dropped). The resulting substrings will be saved in the `"super_q"` and `"sub_q"` columns (which are passed in a vector as the second argument to `separate()`).
```{r freq, echo = 15:20, eval = FALSE}
```
In the current data object, `n` provides the person counts for each of the possible numerical responses in `value`. The output requirements include attaching substantive value labels (e.g., `Excellent`, `Somewhat useful`, `Very little`, etc.) to the numerical responses. This task is complicated by the existence of several different sets of labels, across the super-ordinate question groups, corresponding to the five numerical response choices. The solution requires hard-coded value-label maps, but we can use `stringr` functions to make the code more concise.

We use `mutate(case_when())` to execute this labeling process, creating a new column `label`, and populating it with value labels (strings) that correspond to certain combinations of super-ordinate questions and response values. These combinations are are expressed in predicates evaluated by `case_when()`. 

The next snippet includes the predicate:

`((str_detect(super_q, "Access") | str_detect(super_q, "Quality_of")) & value == 5`

Which operates as follows:

* `(str_detect(super_q, "Access")`: returns `TRUE` for any row where the string in the `super_q` column contains the substring `"Access"`. This substring uniquely identifies a certain super-ordinate question, without having to hard-code the lengthy full text of that question.
* `(str_detect(super_q, "Quality of")`: returns `TRUE` for any row where the string in the `super_q` column contains the substring `"Quality of"`. A second super-ordinate question is thus uniquely identified.
* `|`: the disjunctive operator joins the two calls of `str_detect()`, so that the compound predicate returns `TRUE` for any row contain _either_ of the two substrings, and, consequently, either of two super-ordinate questions that share the same value-label mapping. Thus, another coding efficiency is gained: one set of predicates can be used to apply a certain value-label mapping to all of the super-ordinate questions with which it is associated.
* `& value == 5`: a conjunctive clause is added so that a certain label is applied only to the rows where `value == 5`, within the subset of rows identified by the preceding clause of the predicate.

The predicate constitutes the LHS of the expression evaluated by `case_when()`. The RHS of this expression is `~ "Excellent"`, which codes `label` with `"Excellent"`, but only in the rows isolated by the predicate.
```{r freq, echo = 21:23, eval = FALSE}
```
We can apply column-wise formatting to make the frequency table output more readable. Here is an example of the desired final output:

```
  super_q      sub_q         value label  freq  total_pct valid_pct valid_cum_pct total

 Access/Set… Accessibi… 5     Excel…      20    58.8      58.8      58.8          34   
                        4     Above…      7     20.6      20.6      79.4          34   
                        3     Avera…      7     20.6      20.6     100.0          34   
                        2     Below…      0      0.0       0.0     100.0          34   
                        1     Poor        0      0.0       0.0     100.0          34   
            Accessing_… 5     Excel…      24    70.6      70.6      70.6          34   
                        4     Above…      7     20.6      20.6      91.2          34   
                        3     Avera…      3      8.8       8.8     100.0          34   
                        2     Below…      0      0.0       0.0     100.0          34   
                        1     Poor        0      0.0       0.0     100.0          34  
```
Note that in the `super_q` column, the question text appears only once. In the current data object, that text appears in every cell of the `super_q` column, which clutters the table. The next snippet uses `mutate(case_when())` to recode the `super_q` column to `NA` in every cell except the one where it is needed to concisely label its associated rows. 

As previously, we use a logical test to identify the rows to recode. Here that test is expressed in the predicate `lag(super_q) == super_q`, which returns `TRUE` for rows where the value of a cell in `super_q` is identical to the value of the immediately preceding cell. The only row where that predicate returns `FALSE` is the first instance of a particular super-ordinate question. For those first instances, the preceding cell is either a different super-ordinate question, or `NA` (in the first cell at the top of the column, where there is no preceding cell). `TRUE ~ super_q` is the "else" condition,  indicating (counter-intuitively) that when the predicate returns `FALSE`, the existing value of `super_q` is retained. 

The end result is that the question text is retained in `super_q` only where it first appears, at the top of the set of rows that are associated with that particular super-ordinate question. The `super_q` column is cleared of values in the remainder of that set of rows, creating a more readable table.
```{r freq, echo = 46:50, eval = FALSE}
```
Within `mutate()`, `across()` designates a subset of columns to receive the same formatting or other functional processing. In the next snippet, we format the three `_pct` columns, identifying them in a vector that is the first argument to `across()`. The second argument to `across()` is the formatting function, set off with the formula notation `~`.

Here we use `base::format()` to ensure that percentages will display with one digit to the right of the decimal point. The arguments to `format()` are:

* `.`: a stand-in for the columns identified by `c(total_pct, valid_pct, valid_cum_pct)`;
* `nsmall = 1`: minimum number of digits to display to the right of the decimal point.
```{r freq, echo = 55:56, eval = FALSE}
```
The remainder of this code block completes the formatting of the frequency table output, including the selection and sequencing of desired columns for output, renaming variables, and using `mutate(across(everything))` to `replace_na()` with blank space `""` in all columns.
```{r freq, echo = 57:59, eval = FALSE}
```
At this point, we have generated `freq_table_super_sub_cols`, which is the finalized frequency table output for the super-sub-ordinate question pairs contained in `named_super_sub_r_cols`. We now create similar output for `rhs_num_cols`, which contains the remaining questions with numerical responses. The code here is analogous to that just documented, so only new elements receive commentary. Most of the differences relate to the fact that although both `freq_table_super_sub_cols` and `rhs_num_cols` hold questions with numerical responses, in the former, the original input contains those numerical responses, whereas in the latter, the original input contains the descriptive value labels associated with the numerical reponses, but not the numerical responses themselves.

In the snippet below, we feed `rhs_num_cols` into a pipeline of functions, pivoting it to long format, obtaining person counts for each specific response, and renaming the `value` column to `label`. At this point, the data object has three columns:`item` (question text), `label` (descriptive value label), and `n` (row-wise person count). As mentioned above, the missing column is `value` (numerical response). Note that because of the pivot to long format, the values of `label` are nested within each value of `item`. In other words, each question in the data object has a separate row for each possible descriptive value label, within that question.

Earlier, we created `item_value_label_map`, a lookup table which maps descriptive value labels onto numerical responses. This lookup table was hard-coded to reflect the multiple mapping schemes that exist in the columns of `rhs_num_cols`. Here, we use `right_join()` to perform the lookup operation, using `by =` to align the lookup of `value` to the cell contents of `item` and `label`. As a result, the data object now has the four columns required for the frequency table output: `item`, `label`, `value`, and `n`.
```{r freq, echo = 61:65, eval = FALSE}
```
As in the previous code chunk, we call `mutate(across())` to apply formatting to the `_pct` columns. Here, however, we invoke `ungroup()` to remove the grouping applied earlier in the pipeline, because `across()` often throws an error when it encounters grouped data. We did not need to call `ungroup()` in the previous code chunk, because grouping (apparently) was removed in that pipeline by `separate()`.
```{r freq, echo = 76:83, eval = FALSE}
```
Because we intend to join `freq_table_rhs_num_cols` and `freq_table_super_sub_cols` into a single frequency table, we must ensure that these two objects have identical column structure and formatting. The next call of `mutate()` modifies `freq_table_rhs_num_cols` to meet this requirement, adding an empty `sub_q` column, and using `across(c(value, freq, total), ~ as.character(.))` to coerce a subset of columns to character. `relocate()` puts the columns into the desired sequence.
```{r freq, echo = 86:90, eval = FALSE}
```
As noted at the outset, the final summary report on the course evaluation data is a .xlsx workbook with two sheets: one holding open-ended text responses by participant, and one providing frequency tables for questions with fixed-choice, numerically coded responses. 

The columns and rows needed for the summary table are the product of the code up to this point. We now use `bind_rows()` and `bind_cols` to create two new data frames: 

* `freq_table_all`: the contents of the frequency-table sheet on the final report;
* `text_cols_all`: the contents of the text-response sheet on the final report.

We use `write_xlsx()` to write these two data frames to separate sheets of a single workbook. The first argument to this function is `list(text responses = text_cols_all, freq tables = freq_table_all)`^[When writing code that includes a text label with blank spaces (e.g, `text responses`), we enclose the label in back ticks instead of double-quotes.] `write_xlsx` creates a separate sheet in the output .xlsx for each element of `list()`. The elements of `list()` are expressions that pair a sheet name  (e.g., `text responses`) with a data frame that provides content for that sheet (e.g., `text_cols_all`). The second argument to `write_xlsx()` is the destination file path for the output .xlsx.
```{r freq, echo = 92:106, eval = FALSE}
```
