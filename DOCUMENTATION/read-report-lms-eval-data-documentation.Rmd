...
Here, two calls of `str_detect()` are joined with the conjunctive operator `|`, allowing the predicate to return `TRUE` for any rows where the substrings `"Access"` or `"Quality of"` appear in the `super_q` column. `str_detect()` shortens the code by enabling the identification of certain super-ordinate questions, without having to hard-code the lengthy full text of those questions.

Adding a disjunctive clause with the `&` operator permits the predicate to further isolate only rows where `value == 5`, and to code `label` with `"Excellent"` in these rows.
