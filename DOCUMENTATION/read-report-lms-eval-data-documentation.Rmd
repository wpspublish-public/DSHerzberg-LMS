...
We `map2()` over the two objects, with `.x` holding `list_r_cols`, and `.y` holding `list_col_names`. In each iteration, `.x` supplies a data frame of numerical responses to `set_names()`, which attaches the column names held in `.y`. `map2()` returns a list of data frames whose column names now meet output requirements. By passing this list to `bind_cols()` we return a single data frame, `named_super_sub_r_cols`, which contains all of the super-sub-ordinate question pairs and their numerical responses. This data frame is then processed downstream to create frequency tables.
```{r freq, echo = 48:51, eval = FALSE}
```
